...
function reload() {
	...
	var e = decodeURIComponent(location.hash.substring(1)).replace("~framework.", "~F.").split("~");
...
function render_api(e) {
	...
	e.properties && (t.properties = !0, n = prepare("api", e.name, e.properties), a.push(Tmember({
		...
		name: "Events",
		members: n
	}))), $("#content").html("<h1>" + e.name + '</h1><div class="markdown">' + marked((e.body || "").replace(/‘|’/g, "'")) + "</div>" + Tlinks(t) + a.join("")), highlight()
}
...
function prepare(e, n, a) {
	...
	for (var t = 0, o = a.length; o > t; t++) {
		var r = a[t];
		r.group = n;
		if (r.linker = encodeURIComponent(e + "~" + n + "~" + r.name), r.body = marked(r.body.replace(/‘|’/g, "'")), r.params) {
...
Tangular.register("lower", function(e) {
	...
	for (var a = 0, t = e.length; t > a; a++) {
		...
		o.options && (m = o.options.map(function(e) {
			return "<span>" + e + "</span>"
		})), n.push("<td>" + (m ? m.join("") : "") + (o.body || "") + (o["default"] !== undefined ? '<div class="member-body-default"><b>Default value</b>: ' + o["default"] + "</div>" : "") + (o.params ? o.type.toLowerCase() === 'function' ? "<div><b style=\"font-family:monospace\">function " + o.name + "(" + r + ") {}</b>" + Thelpers.params(o.params) + "</div>" : Thelpers.params(o.params) : "") + "</td>"), n.push("</tr>")
	}
	return n.push("</table>"), n.join("")
}), $.get($("body").attr("data-json"), function(e) {
	documentation = e, (location.hash || "").length < 2 && (location.href = "#" + encodeURIComponent(documentation["default"])), $("#version").html("v" + e.version + (e.build ? " (" + e.build + ")" : "")), configure(), $("#logo").attr("href", "#" + encodeURIComponent(documentation["default"])), reload(), $("#loading").hide(), $("#container").removeClass("hidden")
});
var isFirst = !0,
	isSearch = !1,
...
window.onhashchange = reload, $(document).ready(function() {
	...
	var e, n = location.pathname.lastIndexOf("/"),
		a = location.pathname.substring(n + 1);
	$(".language").find('a[href="' + a + '"]').addClass("selected"), $("#search").bind("keydown", function() {
		...
		var e = this.value.toLowerCase(),
			n = [];
		if ("" !== latest && (location.href = "#"), latest = "", e.length > 2) {
			var a = {};
