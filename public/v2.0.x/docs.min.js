...
function reload() {
	...
	var e = decodeURIComponent(window.location.hash.substring(1)).split("~");
	"#" !== e[0] && "" !== e[0] && (void 0 === e[2] && $("html,body").scrollTop(0), render(e[0], e[1], e[2]))
}

function configure() {
	for (var e = documentation.pages, n = documentation.api, a = documentation.links, t = {}, o = {}, r = [], i = 0, s = e.length; s > i; i++) {
		var m = e[i];
		m.group || (m.group = ""), m.linker = "#" + encodeURIComponent("pages~" + m.name), void 0 === t[m.group] ? t[m.group] = [m] : t[m.group].push(m)
	}
	for (var i = 0, s = n.length; s > i; i++) {
		var m = n[i];
		m.linker = "#" + encodeURIComponent("api~" + m.name), m.group || (m.group = ""), void 0 === o[m.group] ? o[m.group] = [m] : o[m.group].push(m)
	}
	for (var i = 0, s = a.length; s > i; i++) {
		var m = a[i];
		m.linker = m.url, r.push(m)
	}
	var l = Object.keys(t),
		p = $("#menu").empty();
	t[""].push.apply(t[""], r);
	for (var i = 0, s = l.length; s > i; i++) p.append(Tmenu({
		name: l[i],
		items: t[l[i]]
	}));
	l = Object.keys(o);
	...
	$("#menu").find("li").find("a").each(function() {
		var e = $(this),
			n = e.attr("href");
		"http" === n.substring(0, 4) && e.addClass("external")
	})
}

function render(e, n, a) {
	var t = e + "~" + n;
	if (t !== latest) {
		latest = t, setTimeout(function() {
			var t = $('li[data-id="' + e + "~" + encodeURIComponent(n) + "~" + encodeURIComponent(a) + '"]');
			if (0 === t.length) return void $("html,body").scrollTop(0);
			var o = t.offset().top;
			$("html,body").animate({
				scrollTop: o
			}, 100), t.find(".body").removeClass("hidden")
		}, 500), isSearch && $("#search").val("");
		for (var o, r = documentation[e], i = 0, s = r.length; s > i; i++)
			if (r[i].name === n) {
				o = r[i];
				break
			}
		if (o) {
			var m = $("#menu");
			if (m.find(".selected").removeClass("selected"), m.find("li").find('a[href="' + o.linker + '"]').addClass("selected"), void 0 === a && "api" !== e) return document.title = o.name + (o.group ? " (" + o.group + ")" : ""), $("#content").html((o.changelog ? '<div class="member-body-changelog" style="margin-bottom:15px"><b>CHANGELOG:</b> ' + o.changelog + "</div>" : "") + "<h1>" + o.name + '</h1><div class="markdown">' + marked(o.body.replace(/‘|’/g, "'")).replace(/<img src=/g, '<img class="img-responsive" src=') + "</div>"), void highlight();
			document.title = o.name + " (" + o.group + ")", render_api(o), $("a.member").bind("click", function() {
...
function render_api(e) {
	var n, a = [],
		t = {};
	e.properties && (t.properties = !0, n = prepare("api", e.name, e.properties), a.push(Tmember({
		name: "Properties",
		members: n
	}))), e.methods && (t.methods = !0, n = prepare("api", e.name, e.methods), a.push(Tmember({
		name: "Methods",
		members: n
	}))), e.delegates && (t.delegates = !0, n = prepare("api", e.name, e.delegates), a.push(Tmember({
		name: "Delegates",
		members: n
	}))), e.events && (t.events = !0, n = prepare("api", e.name, e.events), a.push(Tmember({
		name: "Events",
		members: n
	}))), $("#content").html("<h1>" + e.name + '</h1><div class="markdown">' + marked((e.description || "").replace(/‘|’/g, "'")) + "</div>" + Tlinks(t) + a.join("")), highlight()
}

function sort(e, n) {
	return e.name && (e = e.name), n.name && (n = n.name), "" === e ? -1 : "" === n ? -1 : e.localeCompare(n)
}

function prepare(e, n, a, isSearch) {
	if (!a) return [];
	a = a.slice(0);
	for (var t = 0, o = a.length; o > t; t++) {
		var r = a[t];
		r.group = n;
		if (r.linker = encodeURIComponent(e + "~" + n + "~" + r.name), r.body = marked(r.description.replace(/‘|’/g, "'")), r.params) {
			for (var i = [], s = 0, m = r.params.length; m > s; s++) {
				var l = r.params[s].options ? -1 !== r.params[s].options.indexOf("optional") : !1;
				i.push((l ? "[" : "") + r.params[s].name + (l ? "]" : ""))
			}
			var p = r.name.indexOf(")"); - 1 !== p ? (r.name = r.name.substring(0, r.name.length - 1), r["return"] || (r["return"] = "Framework"), r.name_params = ", function(" + i.join(", ").replace(/\s$/g, "") + ") { /* body */ })") : (p = r.name.indexOf("}"), -1 !== p && (r.name = r.name.substring(0, r.name.length - 1), r.name_params = "(" + i.join(", ").replace(/\s$/g, "") + ")}")), r.name_params || (r.name_params = "(" + i.join(", ").replace(/\s$/g, "") + ")")
		}
	}
	return a.sort(sort), a
}
...
Tangular.register("lower", function(e) {
	return (e || "").toLowerCase()
}), Tangular.register("params", function(e) {
	var n = [];
	n.push('<table class="table table-bordered table-responsive" cellpadding="0" cellspacing="0">');
	for (var a = 0, t = e.length; t > a; a++) {
		var o = e[a],
			r = "";
		if (n.push("<tr>"), n.push('<td class="active">' + o.name + "<br /><b>" + o.type + "</b></td>"), o.params)
			for (var i = 0; i < o.params.length; i++) {
				var s = o.params[i].options && -1 !== o.params[i].options.indexOf("optional") ? !0 : !1;
				r += ("" !== r ? "," : "") + (s ? "[" : "") + o.params[i].name + (s ? "]" : "")
			}
		var m;
		o.options && (m = o.options.map(function(e) {
			return "<span>" + e + "</span>"
		})), n.push("<td>" + (m ? m.join("") : "") + (o.description || "") + (o["default"] !== undefined ? '<div class="member-body-default"><b>Default value</b>: ' + o["default"] + "</div>" : "") + (o.params ? o.type.toLowerCase() === 'function' ? "<div><b style=\"font-family:monospace\">function " + o.name + "(" + r + ") {}</b>" + Thelpers.params(o.params) + "</div>" : Thelpers.params(o.params) : "") + "</td>"), n.push("</tr>")
	}
	return n.push("</table>"), n.join("")
}), $.get($("body").attr("data-json"), function(e) {
	documentation = e, (window.location.hash || "").length < 2 && (window.location.href = "#" + encodeURIComponent(documentation["default"])), $("#version").html("v" + e.version + (e.build ? " (" + e.build + ")" : "")), configure(), $("#logo").attr("href", "#" + encodeURIComponent(documentation["default"])), reload(), $("#loading").hide(), $("#container").removeClass("hidden")
});
var isFirst = !0,
	isSearch = !1,
	documentation, latest, Tmenu = Tangular.compile($("#template-menu").html()),
	Tmember = Tangular.compile($("#template-api-members").html()),
	Tlinks = Tangular.compile($("#template-api-links").html()),
	Tsearch = Tangular.compile($("#template-search").html());
window.onhashchange = reload, $(document).ready(function() {
	$(document).on("click", "a", function(e) {
		...
		if ("#" === n.attr("href")) {
			...
			var a = n.html();
			$("h2").each(function() {
				this.innerHTML === a && $("html,body").animate({
					scrollTop: $(this).offset().top - 20
				}, 1e3)
			})
		}
	}), $(".offline").toggle(!navigator.onLine), $("#fork").toggle(navigator.onLine);
	var e, n = window.location.pathname.lastIndexOf("/"),
		a = window.location.pathname.substring(n + 1);
	$(".language").find('a[href="' + a + '"]').addClass("selected"), $("#search").bind("keydown", function() {
		"" !== this.value && (clearTimeout(e), e = setTimeout(function() {
			$("#search").trigger("change")
		}, 500))
	}), $("#search").bind("change", function() {
		var e = this.value.toLowerCase(),
			n = [];
		if ("" !== latest && (window.location.href = "#"), latest = "", e.length > 2) {
			var a = {};
			each(documentation.api, function(n) {
				each(n.properties, function(t) {
					search(t, e) && (a[n.name] || (a[n.name] = []), a[n.name].push(t))
				}), each(n.methods, function(t) {
					search(t, e) && (a[n.name] || (a[n.name] = []), a[n.name].push(t))
				}), each(n.delegates, function(t) {
					search(t, e) && (a[n.name] || (a[n.name] = []), a[n.name].push(t))
				}), each(n.events, function(t) {
					search(t, e) && (a[n.name] || (a[n.name] = []), a[n.name].push(t))
				})
			});
			...
			var t = Object.keys(a);
			each(t, function(e) {
				n.push.apply(n, prepare("api", e, a[e]))
			});
		}
		n.length > 100 && (n = n.splice(0, 100)), $("#content").html(Tsearch({
			members: n
		})), isSearch = !0
	})
}), $(window).on("resize", on_resize), on_resize(), $(document).on("click", ".menu-button", function() {
